...
class FormatController(private val preferenceController: PreferenceController) {
    ...
    fun formatSignedAmount(amount: Double): String = (if (amount >= 0.0) "+ " else "- ") + formatAmount(abs(amount))

    fun formatIncome(amount: Double, currency: String): String = (if (amount >= 0) "+ " else "- ") + formatAmount(abs(amount)) + " " + currency

    fun formatExpense(amount: Double, currency: String): String = (if (amount > 0) "+ " else "- ") + formatAmount(abs(amount)) + " " + currency
