...
class RepoModule(private val context: Context) {
    ...
    @Provides
    @Singleton
    fun providesAccountRepo(dbHelper: DbHelper): IRepo<Account> = AccountRepo(dbHelper)

    @Provides
    @Singleton
    fun providesCategoryRepo(dbHelper: DbHelper): IRepo<Category> = CategoryRepo(dbHelper)

    @Provides
    @Singleton
    fun providesExchangeRateRepo(dbHelper: DbHelper): IRepo<ExchangeRate> = ExchangeRateRepo(dbHelper)

    @Provides
    @Singleton
    fun providesRecordRepo(dbHelper: DbHelper): IRepo<Record> = RecordRepo(dbHelper)

    @Provides
    @Singleton
    fun providesTransferRepo(dbHelper: DbHelper): IRepo<Transfer> = TransferRepo(dbHelper)
}
